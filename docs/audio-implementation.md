...
| steady hiss | floor of the > 6 kHz band over all non-silent 100 ms frames: level relative to the music (p5), steadiness (p5 vs p50) and flatness of the quietest-HF frames over 6–16 kHz | mild when constant (p5 within 6 dB of p50), flat (> 0.3) and loud — within 32 dB of the music, or within 38 dB when under 30% of frames are active, since a sparse recording exposes its floor most of the time. A noisy microphone or master. Expected for rain and wind, which measure the same way; a fault for music or birdsong ("Bird sounds in the forest": −30 dB / −1.2 dB / 0.54 with 6% of frames active) |
